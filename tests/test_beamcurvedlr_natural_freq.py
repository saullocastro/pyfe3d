...
from pyfe3d import BeamCurvedLR, BeamCurvedLRData, BeamCurvedLRProbe, DOF, INT, DOUBLE


def test_beamcurvedlr_static_cantilever(refinement=1):
    mtypes = range(2)
    for mtype in mtypes:
        print('mtype', mtype)
        n = 55*refinement
        ...
        for n1, n2 in zip(n1s, n2s):
            ...
            pos2 = nid_pos[n2]
            beam = BeamCurvedLR(p)
            beam.arc_rad = np.deg2rad(arc_deg)/(n-1)
            beam.Rxy = r
        ...
        # comparing arc lengths
        assert np.isclose(np.deg2rad(arc_deg)*r, sum([b.length for b in beams]))
        ...
        both_ends = np.isclose(x, x.min()) | np.isclose(x, x.max())
        bk[0::DOF][both_ends] = True
        bk[1::DOF][both_ends] = True

        # NOTE removing out-of-plane motion
        bk[2::DOF] = True
        bk[3::DOF] = True
        bk[4::DOF] = True
        ...
        omega123_expected_here = [395.21993952, 922.46714619, 1770.03843535]
...
if __name__ == '__main__':
    test_beamcurvedlr_static_cantilever(refinement=1)
