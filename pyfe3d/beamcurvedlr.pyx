...
r"""
BeamCurvedLR - Curved Linear Timoshenko 3D beam element with reduced integration (:mod:`pyfe3d.beamcurvedlr`)
=============================================================================================================

.. currentmodule:: pyfe3d.beamcurvedlr
...
cdef class BeamCurvedLRProbe:
    ...
    def __cinit__(BeamCurvedLRProbe self):
        self.xe = np.zeros(NUM_NODES*DOF//2, dtype=DOUBLE)
        self.ue = np.zeros(NUM_NODES*DOF, dtype=DOUBLE)


cdef class BeamCurvedLR:
    r"""
    Curved Timoshenko 3D beam element with linear shape functions
    ...
    length, : double
        Element length.
    Rxy, : double
        Element radius in the plane xy of the local coordinates.
    arc_rad, : double
        Total arc angle in radians covered by the beam element.
    ...
    init_k_KC0, init_k_KG, init_k_M : int
        ...
        matrices.
    probe : :class:`.BeamCurvedLRProbe` object
    ...
    cdef public double length, Rxy, arc_rad
    cdef public double r11, r12, r13, r21, r22, r23, r31, r32, r33
    cdef public BeamCurvedLRProbe probe

    def __cinit__(BeamCurvedLR self, BeamCurvedLRProbe p):
        self.probe = p
        self.eid = -1
        self.Rxy = 0
        self.arc_rad = 0
    ...
    cpdef void update_rotation_matrix(BeamCurvedLR self, double vxyi, double
            vxyj, double vxyk, np.ndarray[cDOUBLE, ndim=1] x):
    ...
    cpdef void update_probe_ue(BeamCurvedLR self, np.ndarray[cDOUBLE, ndim=1] u):
        r"""Update the local displacement vector of the probe of the element

        .. note:: The ``probe`` attribute object :class:`.BeamCurvedLRProbe` is
    ...
    cpdef void update_probe_xe(BeamCurvedLR self, np.ndarray[cDOUBLE, ndim=1] x):
    ...
    cpdef void update_length(BeamCurvedLR self):
        r"""Update element length

        """
        with nogil:
            self.length = self.arc_rad*self.Rxy


    cpdef void update_KC0(BeamCurvedLR self,
        ...
        """
        cdef cINT c1, c2, k
        cdef double L, A, E, G, Ay, Az, Iyy, Izz, Iyz, J, Rxy
        ...
        with nogil:
            ...
            k = self.init_k_KC0
            KC0v[k] += 1.0*A*G*r13**2/L + r11*(0.5*A*r12*(-E + G)/Rxy + 1.0*L*r11*(A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r11*(-E + G)/Rxy + 1.0*L*r12*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r13*r23/L + r21*(0.5*A*r12*(-E + G)/Rxy + 1.0*L*r11*(A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r11*(-E + G)/Rxy + 1.0*L*r12*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r13*r33/L + r31*(0.5*A*r12*(-E + G)/Rxy + 1.0*L*r11*(A*E/L**2 + A*G/(4*Rxy**2))) + r32*(0.5*A*r11*(-E + G)/Rxy + 1.0*L*r12*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += r11*(1.0*Ay*G*r13/L - 0.5*Az*G*r11/Rxy - 1.0*Az*G*r12/L) + r12*(-0.5*A*G*r13 - 0.5*Az*E*r12/Rxy + 1.0*Az*E*r11/L) + r13*(1.0*L*r11*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r12*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r21*(1.0*Ay*G*r13/L - 0.5*Az*G*r11/Rxy - 1.0*Az*G*r12/L) + r22*(-0.5*A*G*r13 - 0.5*Az*E*r12/Rxy + 1.0*Az*E*r11/L) + r23*(1.0*L*r11*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r12*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r31*(1.0*Ay*G*r13/L - 0.5*Az*G*r11/Rxy - 1.0*Az*G*r12/L) + r32*(-0.5*A*G*r13 - 0.5*Az*E*r12/Rxy + 1.0*Az*E*r11/L) + r33*(1.0*L*r11*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r12*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += -1.0*A*G*r13**2/L + r11*(0.5*A*r12*(E + G)/Rxy + 1.0*L*r11*(-A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r11*(-E - G)/Rxy + 1.0*L*r12*(A*E/(4*Rxy**2) - A*G/L**2))
            k += 1
            KC0v[k] += -1.0*A*G*r13*r23/L + r21*(0.5*A*r12*(E + G)/Rxy + 1.0*L*r11*(-A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r11*(-E - G)/Rxy + 1.0*L*r12*(A*E/(4*Rxy**2) - A*G/L**2))
            ...
            KC0v[k] += r31*(-1.0*Ay*G*r13/L + 0.5*Az*G*r11/Rxy + 1.0*Az*G*r12/L) + r32*(-0.5*A*G*r13 + 0.5*Az*E*r12/Rxy - 1.0*Az*E*r11/L) + r33*(1.0*L*r11*(A*G/(4*Rxy) + Ay*E/L**2) + 0.5*r12*(A*G*Rxy - Ay*E)/Rxy)
            k += 1
            KC0v[k] += 1.0*A*G*r13*r23/L + r11*(0.5*A*r22*(-E + G)/Rxy + 1.0*L*r21*(A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r21*(-E + G)/Rxy + 1.0*L*r22*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r23**2/L + r21*(0.5*A*r22*(-E + G)/Rxy + 1.0*L*r21*(A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r21*(-E + G)/Rxy + 1.0*L*r22*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r23*r33/L + r31*(0.5*A*r22*(-E + G)/Rxy + 1.0*L*r21*(A*E/L**2 + A*G/(4*Rxy**2))) + r32*(0.5*A*r21*(-E + G)/Rxy + 1.0*L*r22*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += r11*(1.0*Ay*G*r23/L - 0.5*Az*G*r21/Rxy - 1.0*Az*G*r22/L) + r12*(-0.5*A*G*r23 - 0.5*Az*E*r22/Rxy + 1.0*Az*E*r21/L) + r13*(1.0*L*r21*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r22*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r21*(1.0*Ay*G*r23/L - 0.5*Az*G*r21/Rxy - 1.0*Az*G*r22/L) + r22*(-0.5*A*G*r23 - 0.5*Az*E*r22/Rxy + 1.0*Az*E*r21/L) + r23*(1.0*L*r21*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r22*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r31*(1.0*Ay*G*r23/L - 0.5*Az*G*r21/Rxy - 1.0*Az*G*r22/L) + r32*(-0.5*A*G*r23 - 0.5*Az*E*r22/Rxy + 1.0*Az*E*r21/L) + r33*(1.0*L*r21*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r22*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += -1.0*A*G*r13*r23/L + r11*(0.5*A*r22*(E + G)/Rxy + 1.0*L*r21*(-A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r21*(-E - G)/Rxy + 1.0*L*r22*(A*E/(4*Rxy**2) - A*G/L**2))
            k += 1
            KC0v[k] += -1.0*A*G*r23**2/L + r21*(0.5*A*r22*(E + G)/Rxy + 1.0*L*r21*(-A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r21*(-E - G)/Rxy + 1.0*L*r22*(A*E/(4*Rxy**2) - A*G/L**2))
            ...
            KC0v[k] += r31*(-1.0*Ay*G*r23/L + 0.5*Az*G*r21/Rxy + 1.0*Az*G*r22/L) + r32*(-0.5*A*G*r23 + 0.5*Az*E*r22/Rxy - 1.0*Az*E*r21/L) + r33*(1.0*L*r21*(A*G/(4*Rxy) + Ay*E/L**2) + 0.5*r22*(A*G*Rxy - Ay*E)/Rxy)
            k += 1
            KC0v[k] += 1.0*A*G*r13*r33/L + r11*(0.5*A*r32*(-E + G)/Rxy + 1.0*L*r31*(A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r31*(-E + G)/Rxy + 1.0*L*r32*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r23*r33/L + r21*(0.5*A*r32*(-E + G)/Rxy + 1.0*L*r31*(A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r31*(-E + G)/Rxy + 1.0*L*r32*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += 1.0*A*G*r33**2/L + r31*(0.5*A*r32*(-E + G)/Rxy + 1.0*L*r31*(A*E/L**2 + A*G/(4*Rxy**2))) + r32*(0.5*A*r31*(-E + G)/Rxy + 1.0*L*r32*(A*E/(4*Rxy**2) + A*G/L**2))
            k += 1
            KC0v[k] += r11*(1.0*Ay*G*r33/L - 0.5*Az*G*r31/Rxy - 1.0*Az*G*r32/L) + r12*(-0.5*A*G*r33 - 0.5*Az*E*r32/Rxy + 1.0*Az*E*r31/L) + r13*(1.0*L*r31*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r32*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r21*(1.0*Ay*G*r33/L - 0.5*Az*G*r31/Rxy - 1.0*Az*G*r32/L) + r22*(-0.5*A*G*r33 - 0.5*Az*E*r32/Rxy + 1.0*Az*E*r31/L) + r23*(1.0*L*r31*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r32*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += r31*(1.0*Ay*G*r33/L - 0.5*Az*G*r31/Rxy - 1.0*Az*G*r32/L) + r32*(-0.5*A*G*r33 - 0.5*Az*E*r32/Rxy + 1.0*Az*E*r31/L) + r33*(1.0*L*r31*(A*G/(4*Rxy) - Ay*E/L**2) + 0.5*r32*(A*G*Rxy + Ay*E)/Rxy)
            k += 1
            KC0v[k] += -1.0*A*G*r13*r33/L + r11*(0.5*A*r32*(E + G)/Rxy + 1.0*L*r31*(-A*E/L**2 + A*G/(4*Rxy**2))) + r12*(0.5*A*r31*(-E - G)/Rxy + 1.0*L*r32*(A*E/(4*Rxy**2) - A*G/L**2))
            k += 1
            KC0v[k] += -1.0*A*G*r23*r33/L + r21*(0.5*A*r32*(E + G)/Rxy + 1.0*L*r31*(-A*E/L**2 + A*G/(4*Rxy**2))) + r22*(0.5*A*r31*(-E - G)/Rxy + 1.0*L*r32*(A*E/(4*Rxy**2) - A*G/L**2))
            ...
            KC0v[k] += r31*(-1.0*Ay*G*r33/L + 0.5*Az*G*r31/Rxy + 1.0*Az*G*r32/L) + r32*(-0.5*A*G*r33 + 0.5*Az*E*r32/Rxy - 1.0*Az*E*r31/L) + r33*(1.0*L*r31*(A*G/(4*Rxy) + Ay*E/L**2) + 0.5*r32*(A*G*Rxy - Ay*E)/Rxy)
            k += 1
            KC0v[k] += r11*(1.0*Az*E*r12/L - 0.5*Az*G*r11/Rxy + 1.0*L*r13*(A*G/(4*Rxy) - Ay*E/L**2)) + r12*(-0.5*Az*E*r12/Rxy - 1.0*Az*G*r11/L + 0.5*r13*(A*G*Rxy + Ay*E)/Rxy) + r13*(-0.5*A*G*r12 + 1.0*Ay*G*r11/L)
            k += 1
            KC0v[k] += r21*(1.0*Az*E*r12/L - 0.5*Az*G*r11/Rxy + 1.0*L*r13*(A*G/(4*Rxy) - Ay*E/L**2)) + r22*(-0.5*Az*E*r12/Rxy - 1.0*Az*G*r11/L + 0.5*r13*(A*G*Rxy + Ay*E)/Rxy) + r23*(-0.5*A*G*r12 + 1.0*Ay*G*r11/L)
            k += 1
            KC0v[k] += r31*(1.0*Az*E*r12/L - 0.5*Az*G*r11/Rxy + 1.0*L*r13*(A*G/(4*Rxy) - Ay*E/L**2)) + r32*(-0.5*Az*E*r12/Rxy - 1.0*Az*G*r11/L + 0.5*r13*(A*G*Rxy + Ay*E)/Rxy) + r33*(-0.5*A*G*r12 + 1.0*Ay*G*r11/L)
            k += 1
            ...
            KC0v[k] += r31*(0.5*Ay*G*r12 + 0.5*Az*G*r13 - 1.0*G*J*r11/L) + r32*(-0.5*Ay*G*r11 + 1.0*E*Iyz*r13/L + 1.0*L*r12*(A*G/4 - E*Iyy/L**2)) + r33*(-0.5*Az*G*r11 + 1.0*E*Iyz*r12/L + 1.0*L*r13*(A*G/4 - E*Izz/L**2))
            k += 1
            KC0v[k] += r11*(1.0*Az*E*r22/L - 0.5*Az*G*r21/Rxy + 1.0*L*r23*(A*G/(4*Rxy) - Ay*E/L**2)) + r12*(-0.5*Az*E*r22/Rxy - 1.0*Az*G*r21/L + 0.5*r23*(A*G*Rxy + Ay*E)/Rxy) + r13*(-0.5*A*G*r22 + 1.0*Ay*G*r21/L)
            k += 1
            KC0v[k] += r21*(1.0*Az*E*r22/L - 0.5*Az*G*r21/Rxy + 1.0*L*r23*(A*G/(4*Rxy) - Ay*E/L**2)) + r22*(-0.5*Az*E*r22/Rxy - 1.0*Az*G*r21/L + 0.5*r23*(A*G*Rxy + Ay*E)/Rxy) + r23*(-0.5*A*G*r22 + 1.0*Ay*G*r21/L)
            k += 1
            KC0v[k] += r31*(1.0*Az*E*r22/L - 0.5*Az*G*r21/Rxy + 1.0*L*r23*(A*G/(4*Rxy) - Ay*E/L**2)) + r32*(-0.5*Az*E*r22/Rxy - 1.0*Az*G*r21/L + 0.5*r23*(A*G*Rxy + Ay*E)/Rxy) + r33*(-0.5*A*G*r22 + 1.0*Ay*G*r21/L)
            k += 1
            ...
            KC0v[k] += r31*(0.5*Ay*G*r22 + 0.5*Az*G*r23 - 1.0*G*J*r21/L) + r32*(-0.5*Ay*G*r21 + 1.0*E*Iyz*r23/L + 1.0*L*r22*(A*G/4 - E*Iyy/L**2)) + r33*(-0.5*Az*G*r21 + 1.0*E*Iyz*r22/L + 1.0*L*r23*(A*G/4 - E*Izz/L**2))
            k += 1
            KC0v[k] += r11*(1.0*Az*E*r32/L - 0.5*Az*G*r31/Rxy + 1.0*L*r33*(A*G/(4*Rxy) - Ay*E/L**2)) + r12*(-0.5*Az*E*r32/Rxy - 1.0*Az*G*r31/L + 0.5*r33*(A*G*Rxy + Ay*E)/Rxy) + r13*(-0.5*A*G*r32 + 1.0*Ay*G*r31/L)
            k += 1
            KC0v[k] += r21*(1.0*Az*E*r32/L - 0.5*Az*G*r31/Rxy + 1.0*L*r33*(A*G/(4*Rxy) - Ay*E/L**2)) + r22*(-0.5*Az*E*r32/Rxy - 1.0*Az*G*r31/L + 0.5*r33*(A*G*Rxy + Ay*E)/Rxy) + r23*(-0.5*A*G*r32 + 1.0*Ay*G*r31/L)
            k += 1
            KC0v[k] += r31*(1.0*Az*E*r32/L - 0.5*Az*G*r31/Rxy + 1.0*L*r33*(A*G/(4*Rxy) - Ay*E/L**2)) + r32*(-0.5*Az*E*r32/Rxy - 1.0*Az*G*r31/L + 0.5*r33*(A*G*Rxy + Ay*E)/Rxy) + r33*(-0.5*A*G*r32 + 1.0*Ay*G*r31/L)
            k += 1
    ...
    cpdef void update_M(BeamCurvedLR self,
